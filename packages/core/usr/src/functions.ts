...
export function md5(inputString: string):string {
  ...
  let i, a = 1732584193, b = -271733879, c = -1732584194, d = 271733878, olda, oldb, oldc,
    oldd;
...
/**
 * Remove backslashes in the string
 * @param inputString Input string
 */
export function stripSlashes(inputString: string): string {
  return inputString.replace(/\\(.)/g, (match, char) => {
    return ({'t': '\t', 'r': '\r', 'n': '\n', '0': '\0'} as Record<string, string>)[char] || char;
  })
}

/**
 * Throw error.
 * It is used in inline expressions, because throw keyword is not allowed.
 * Example:
 *  null ?? error(new Error(`Must be not null!`))
 * @param error Error
 */
 ...
 * @param obj Object
 */
function checkGlobal(obj: false | undefined | { Math?: unknown }) {
  // Math must exist in a global environment
